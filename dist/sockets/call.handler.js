...
const registerCallHandlers = (io, socket) => {
    ...
    socket.on('call:initiate', async (payload, callback) => {
        try {
            ...
            const receiverPresence = await redis_service_1.redisService.getPresence(receiverId);
            if (receiverPresence === 'offline') {
                // If receiver is offline, mark call as missed immediately
                const call = await db_service_1.prisma.call.create({
                    data: {
                        callerId: userId,
                        receiverId,
                        status: 'missed',
                ...
                if (callback)
                    callback({ success: false, error: 'User is offline', callId: call.id });
                return;
            }
            ...
            await redis_service_1.redisService.setCallState(call.id, callState);
            // Notify recipient
            ...
            // Start Call Timeout
            setTimeout(async () => {
                const currentCallState = await redis_service_1.redisService.getCallState(call.id);
                if (currentCallState && (currentCallState.status === 'calling' || currentCallState.status === 'ringing')) {
                    // Timeout occurred - mark as missed
                    await db_service_1.prisma.call.update({
                        where: { id: call.id },
                        data: {
                            status: 'missed',
                            endedAt: new Date(),
                    ...
                    await redis_service_1.redisService.deleteCallState(call.id);
            ...
            if (callback)
                callback({ success: false, error: 'Internal server error initiating call' });
        }
    });
    // 2. Ringing State
    socket.on('call:ringing', async (payload) => {
        try {
            ...
            const callState = await redis_service_1.redisService.getCallState(callId);
            if (!callState || callState.receiverId !== userId)
                return;
            // Update call status
            callState.status = 'ringing';
            await redis_service_1.redisService.setCallState(callId, callState);
            await db_service_1.prisma.call.update({
                where: { id: callId },
                data: { status: 'ringing' },
            });
    ...
    socket.on('call:accept', async (payload, callback) => {
        try {
            ...
            if (!callState || callState.receiverId !== userId) {
                if (callback)
                    callback({ success: false, error: 'Call not found or unauthorized' });
                return;
            }
            callState.status = 'connected';
            await redis_service_1.redisService.setCallState(callId, callState);
            const now = new Date();
            const updatedCall = await db_service_1.prisma.call.update({
                where: { id: callId },
                data: {
                    status: 'connected',
                    answeredAt: now,
            ...
            if (callback)
                callback({ success: true, data: updatedCall });
        }
        catch (err) {
    ...
    socket.on('call:reject', async (payload) => {
        try {
            ...
            await redis_service_1.redisService.deleteCallState(callId);
            // Notify caller
    ...
    socket.on('call:cancel', async (payload) => {
        try {
            const { callId } = payload;
            const callState = await redis_service_1.redisService.getCallState(callId);
            if (!callState || callState.callerId !== userId)
                return;
            await db_service_1.prisma.call.update({
                where: { id: callId },
                data: {
    ...
    socket.on('call:end', async (payload) => {
        try {
            ...
            const call = await db_service_1.prisma.call.findUnique({ where: { id: callId } });
